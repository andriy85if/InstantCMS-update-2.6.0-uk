...
function install_package(){
    ...
    if(!$core->db->getRowsCount('controllers', "name = 'redirect'")){
        $core->db->query("INSERT INTO `{#}controllers` (`title`, `name`, `is_enabled`, `options`, `author`, `url`, `version`, `is_backend`, `is_external`) VALUES ('Редиректи', 'redirect', 1, '---\nno_redirect_list:\nblack_list:\nis_check_link: null\nwhite_list:\nredirect_time: 10\n', 'InstantCMS Team', 'http://www.instantcms.ru', '2.0', 1, NULL);");
    }
    if(!$core->db->getRowsCount('controllers', "name = 'commentsvk'")){
        $core->db->query("INSERT INTO `{#}controllers` (`title`, `name`, `is_enabled`, `options`, `author`, `url`, `version`, `is_backend`, `is_external`) VALUES ('Коментарі Вконтакті', 'commentsvk', 1, '---\napi_id: \nredesign: null\nautoPublish: 1\nnorealtime: null\nmini: 0\nattach:\n  - graffiti\n  - photo\n  - video\n  - audio\nlimit: 50\n', 'InstantCMS Team', 'http://www.instantcms.ru', '1.0', 1, NULL)");
    }
    if(!$core->db->getRowsCount('controllers', "name = 'geo'")){
        $core->db->query("INSERT INTO `{#}controllers` (`title`, `name`, `is_enabled`, `options`, `author`, `url`, `version`, `is_backend`, `is_external`) VALUES ('Географія', 'geo', 1, NULL, 'InstantCMS Team', 'http://www.instantcms.ru', '2.0', 1, NULL)");
    }

    if(!$core->db->getRowsCount('activity_types', "name = 'vote.comment'")){
        $core->db->query("INSERT INTO `{#}activity_types` (`is_enabled`, `controller`, `name`, `title`, `description`) VALUES (1, 'comments', 'vote.comment', 'Оцінка коментаря', 'оцінив коментар на сторінці %s');");
    }

    if(!$core->db->getRowsCount('scheduler_tasks', "controller = 'messages'")){
        $core->db->query("INSERT INTO `{#}scheduler_tasks` (`title`, `controller`, `hook`, `period`, `is_active`) VALUES ('Очищення видалених особистих повідомлень', 'messages', 'clean', '1440', '1');");
    }

    if(!$core->db->getRowsCount('scheduler_tasks', "controller = 'auth' AND hook = 'delete_expired_unverified'")){
        $core->db->query("INSERT INTO `{#}scheduler_tasks` (`title`, `controller`, `hook`, `period`, `is_active`) VALUES ('Видалення користувачів, які не пройшли верифікацію', 'auth', 'delete_expired_unverified', '60', '1');");
    }
    ...
    if(!isFieldExists('{users}_messages', 'date_delete')){
        $core->db->query("ALTER TABLE `{users}_messages` ADD `date_delete` TIMESTAMP NULL DEFAULT NULL COMMENT 'Дата видалення' AFTER `date_pub`, ADD INDEX (`date_delete`);");
    }

    $core->db->query("ALTER TABLE `{#}comments` CHANGE `author_url` `author_url` VARCHAR( 15 ) NULL DEFAULT NULL COMMENT 'ip адреса'");
    $core->db->query("ALTER TABLE `{users}_messages` CHANGE `date_pub` `date_pub` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Дата створення'");
    ...
    if(!$core->db->getRowsCount('widgets', "controller = 'photos' AND name = 'list'")){
        $core->db->query("INSERT INTO `{#}widgets` (`controller`, `name`, `title`, `author`, `url`, `version`) VALUES ('photos', 'list', 'Список фотографій', 'InstantCMS Team', 'http://www.instantcms.ru', '2.0');");
    }
